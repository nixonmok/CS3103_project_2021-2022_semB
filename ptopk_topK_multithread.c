...
{
    int nproc = MIN(get_nprocs(), 4);
    pthread_t *threads = malloc(sizeof(pthread_t) * nproc);
    int left = number_of_hours % nproc;
    int part = (int)(number_of_hours / nproc);
    int *endidxs = malloc(sizeof(int) * part);

    for (int idx = 0; idx < nproc; ++idx) {
        struct threadargument *arg = malloc(sizeof(struct threadargument));

        arg->k = k;
        arg->timeWithCountArr = timeWithCountArr;
        arg->start = idx == 0 ? 0: part * idx;
        arg->end = idx + 1 == nproc ? part + left: part;
        endidxs[idx] = arg->end;

        if (pthread_create(threads + idx, NULL, consumer, arg) != 0) {
...
int main(int argc, char **argv) //**argv = console command, e.g (./file.c 1 2)
{
    ...
    //printf("read file cost sec: %ld\n", end.tv_sec - start.tv_sec);
    //printf("read file cost usec: %ld\n", end.tv_usec - start.tv_usec);

    producer(atoi(argv[3]), number_of_hours);
